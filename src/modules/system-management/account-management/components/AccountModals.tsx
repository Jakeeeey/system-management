...
import { ShieldAlert, Key, CheckCircle2, Mail, Info } from "lucide-react"

interface ModalProps {
    isOpen: boolean;
    onClose: () => void;
    user: AccountUser | null;
    onConfirm: (data: { userId: number; reason?: string }) => Promise<boolean>;
}
...
export function PasswordResetOptionsModal({ isOpen, onClose, onSelect }: { isOpen: boolean; onClose: () => void; onSelect: (action: 'FORCE_RESET' | 'SEND_RESET') => void }) {
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[850px] bg-white dark:bg-[#0c0c14] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white p-0 overflow-hidden shadow-2xl">
